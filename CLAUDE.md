...
| Memory | `MemoryStore.swift` (glossary + notes + extracted names, human-editable memory.md in App Support), `MemoryCommands.swift` (voice teach/recall intents), `MemoryBias.swift` (confirm-card logic), `MemoryConfirmView.swift` (fact confirmation card), `MemorySettingsView.swift` |
...
Memory lives in `~/Library/Application Support/<bundle id>/memory.md` (NOT UserDefaults) — Claude-style human-editable markdown: `## Vocabulary` / `## Facts` / `## Names` / `## Settings` sections, one `- ` item per line; parsed leniently (hand edits can never wipe it); legacy memory.json auto-migrates on first load. extractedNames also join the cleanup prompt's spelling list. (An acoustic CTC-biasing layer was built and removed 2026-07-25 — see ParakeetTranscriptionService for why; don't re-add without an offline eval harness.)
